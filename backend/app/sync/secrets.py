...
async def save_google_oauth_secret(secret: dict[str, Any]) -> None:
...
async def load_google_oauth_secret(*, required: bool = True) -> dict[str, Any]:
    path = GOOGLE_OAUTH_CREDENTIALS_PATH
...
def delete_google_oauth_secret() -> bool:
    path = GOOGLE_OAUTH_CREDENTIALS_PATH
    existed = path.exists()
